"""
Provides a logger class for logging messages to both console and file.
"""
...
_logger_argparser.add_argument(
    "--log_dir",
    type=str,
    nargs="?",
    default=argparse.SUPPRESS,
    help="Path to dir that stores log files",
)
...
_logger_argparser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Create log directory if it does not exist",
)
...
class Logger:
    ...
    def __init__(self) -> None:
        ...
        # default log entry format for file logger and stderr logger
        self.__log_file_format = "[%(asctime)s](%(filename)s:%(lineno)d) %(levelname)s %(message)s"
        self.__log_stderr_format = "%(levelname)s %(message)s"

        # initialize the root logger for all modules using this utils
        self.__default_logger = logging.getLogger("Utils Root Logger")
        if hasattr(_logger_args, "log_dir"):
            logging_folder = putil.safeval(_logger_args.log_dir, _logger_default_log_dir)

            if not os.path.isdir(logging_folder):
                if not _logger_args.create_log_dir:
                    cprint.eprintf(
                        f"{self.__default_logger.name} - "
            ...
            self.__default_logger.addHandler(stderr_handler)
    ...
    @property
    def default_logging_level_name(self) -> str:
        return logging.getLevelName(self.__default_logger.level)
    ...
    def __convert_name_to_readable(self, name: str) -> str:
        name = name.replace("_", " ").replace("-", " ").strip()
        if " " in name:
            # for names with snake_case or kebab-case
            return " ".join([f"{word[0].upper()}{word[1:]}" for word in name.split()])
        else:
    ...
    def __get_readable_name(self, comp_name: str, name_level: int) -> str:
        ...
        for _ in range(name_level):
            dir_basename = os.path.basename(comp_dir)
            dir_names.append(self.__convert_name_to_readable(dir_basename))
    ...
    def register_component(
        ...
        Args:
            ...
            `auto_readable`: whether to convert the component name to a more human-readable form
                (default True)
            `name_level`: level of name to be returned, indicating number of directory levels
                included in front of the component name (default 0)
...
def addLoggingLevel(levelName, levelNum, methodName=None):
    ...
    Reference: https://stackoverflow.com/questions/2183233/how-to-add-a-custom-loglevel-to-pythons-logging-facility/35804945#35804945
    """
    if not methodName:
